...
st.set_page_config(page_title="Fuzzy Set Markov Engine", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🌀 Fuzzy Set Markov Engine</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Fuzzy C‑Means clustering of returns | Membership in highest‑variance regime</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Fuzzy Markov")
...
st.sidebar.markdown(f"**Fuzzy clusters:** {config.N_CLUSTERS} | **Fuzziness m:** {config.FUZZINESS}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'fuzzy_markov_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Extreme regime membership: {etf['fuzzy_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Membership"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Membership", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🌀 Top ETFs by Fuzzy Membership in Extreme Regime (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Fuzzy C‑Means** clustering assigns each day's return a membership degree to multiple regimes.
        - We identify the regime with the highest variance (most volatile) as the "extreme" state.
        - The score for an ETF is its membership degree in that extreme regime on the last day of the window.
        - High membership → ETF is currently in a high‑volatility, potentially rewarding regime.
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the fuzzy membership scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Fuzzy Set Markov | Fuzzy C‑Means clustering for regime detection")
