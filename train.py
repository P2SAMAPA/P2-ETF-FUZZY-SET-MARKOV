...
from fuzzy_markov import fuzzy_markov_score
...
def run_for_window(returns, window_days):
    ...
    top_etfs = [{"ticker": t, "fuzzy_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    out_file = f"output/fuzzy_markov_{timestamp}.json"
